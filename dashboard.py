...
for _, row in filtered.iterrows():
    ...
    alert_html = f"⚠️ {abnorm}" if abnorm != "None" else ""
    rec_short = rec[:100] + ("..." if len(rec) > 100 else "")
...
<div class="patient-card {css_cls}">
  <div style="display:flex; justify-content:space-between; align-items:center;">
    <span style="font-size:1.05rem; font-weight:700;">
      {icon} {row['patient_id']} &nbsp;
      <span style="color:#888; font-size:0.85rem;">{row['case_type']}</span>
    </span>
  ...
  <div style="margin-top:4px; font-size:0.78rem; color:#e74c3c;">{alert_html}</div>
  <div style="margin-top:3px; font-size:0.78rem; color:#7ecfaa;">💊 {rec_short}</div>
